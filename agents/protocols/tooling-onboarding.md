...
The audit inspects local commands, Codex skills, Codex plugins, and MCP-related capabilities. It does not install, download, connect, or mutate anything. Its JSON output reports `next_actions` for permission-gated setup.

## Permission Flow

- Read `summary`, `items`, and `next_actions`.
- Ask the user for permission before every local tool install, GitHub-backed skill/resource download, Codex plugin/MCP connection, or frontend dependency addition.
